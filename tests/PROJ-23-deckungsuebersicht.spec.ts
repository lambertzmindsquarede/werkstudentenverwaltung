...
test('Tagesansicht Monday — previous day button is disabled (no earlier weekday)', async ({ page, browser }) => {
  ...
  await ctx.addCookies(managerCookies)
  const p = await ctx.newPage()
  // Monday 2026-05-04 (2026-W19 starts on Monday May 4)
  await p.goto('/manager/deckung?week=2026-W19&view=tag&day=2026-05-04')
  await p.waitForLoadState('networkidle')
  await p.waitForTimeout(500)
  // The prev-day button in TagesGantt is disabled on Monday (Sunday is not a weekday)
  const tagesTabPanel = p.locator('[role="tabpanel"][data-state="active"]')
  const disabledBtn = tagesTabPanel.locator('button[disabled]').first()
...
test('Tagesansicht Friday — next day button is disabled', async ({ page, browser }) => {
  ...
  const p = await ctx.newPage()
  // Friday 2026-05-08
  await p.goto('/manager/deckung?week=2026-W19&view=tag&day=2026-05-08')
  await p.waitForLoadState('networkidle')
  await p.waitForTimeout(500)
...
test('next week button updates URL to KW+1', async ({ page, browser }) => {
  ...
  await p.goto('/manager/deckung?week=2026-W19')
  await p.waitForLoadState('networkidle')
  // Scope to the week-nav container (parent of the .min-w-36 KW display div)
  const weekNavContainer = p.locator('.min-w-36').locator('..')
  await weekNavContainer.getByRole('button').last().click()
...
test('previous week button updates URL to KW-1', async ({ page, browser }) => {
  ...
  // Scope to the week-nav container (parent of the .min-w-36 KW display div)
  const weekNavContainer = p.locator('.min-w-36').locator('..')
  await weekNavContainer.getByRole('button').first().click()
